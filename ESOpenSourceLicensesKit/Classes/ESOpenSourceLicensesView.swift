...
*/
open class ESOpenSourceLicensesView : WKWebView {
    ...
    */
    override open func reload() -> WKNavigation? {
        do {
            ...
            let modifiedString = regex.stringByReplacingMatches(in: contents as String, options: .withoutAnchoringBounds, range: NSMakeRange(0, contents.count), withTemplate: template as String)
            return self.loadHTMLString(modifiedString, baseURL: nil)
        } catch {
            print("ESOpenSourceLicensesView reload error: \(error)")
            return nil
        }
    }
